...
void configInit(void) {
    ...
    configLoadDefault();                //默认配置读入P数组
    
    ver = *(float *)FLASH_WRITE_ADDR;   //读取版本号的地址

    if (isnan(ver))											//判断ver，根据大小写flash
		configWriteFlash();                 
...
int configGetId(char *param) {
	...
	for (i = 0; i < CONFIG_NUM_PARAMS; i++)
		if (!strncasecmp(configParameterStrings[i], param, strlen(configParameterStrings[i])))//锟街凤拷锟斤拷锟饺斤拷,锟饺较成癸拷锟斤拷锟斤拷锟斤拷锟斤拷index
			return i;
